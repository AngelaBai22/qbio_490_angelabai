...
```{r}
write.csv(clinic, "C:/Users/15713/Desktop/QBio490/qbio_490_angelabai/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```
...
# 2. What is the data type of $gender? Boolean, as there are only two possible outputs 
# 3. What is the age in years of the 5th patient (barcode: TCGA-4H-AAAK)? 50 years old
# 4. How old was the 1st patient at their last follow up (barcode: TCGA-3C-AAAU)? HINT: you need to look at data from two columns to answer this. 44 years old
...
```{r Exercise 4.1}
# create plot here
x <- clinic$birth_days_to
y <- clinic$lymph_nodes_examined_he_count

plot(x, y, main = "Number of Lymph Nodes as a Function of Patient Age",
     xlab = "Age (days)", ylab = "# of Lymph Nodes",
     pch = 19, frame = FALSE)
...
clinic$Tumor_Sample_Barcode[male_mask]

```

3) Create a mask to select for patients under the age of 35 OR over 60 at diagnosis. 
```{r}
age_at_diagnosis_na_mask <- is.na(clinic$age_at_diagnosis)

under30_over60_mask <- ifelse(35 > clinic$age_at_diagnosis, TRUE, ifelse (60 < clinic$age_at_diagnosis, TRUE, FALSE))
...
clinic$no_middle_clinic <- ifelse(clinic$age_at_diagnosis > 35 & clinic$age_at_diagnosis < 60, FALSE, TRUE)
...
```{r}
age_NA_mask <- ifelse(is.na(clinic$age_at_diagnosis), FALSE, TRUE)
...
```{r}
young_mask <- ifelse(age_cleaned_clinical$age_at_diagnosis < 35, TRUE, FALSE)
middle_mask <- ifelse(age_cleaned_clinical$age_at_diagnosis >= 35 & age_cleaned_clinical$age_at_diagnosis <= 50, TRUE, FALSE)

old_mask <- ifelse(age_cleaned_clinical$age_at_diagnosis > 50, TRUE, FALSE)
age_cleaned_clinical$age_category <- ifelse(young_mask, "Young", ifelse(middle_mask, "Middle", "Old"))

```

```{r}
View(age_cleaned_clinical$age_ca)
```
...
age_cleaned_clinical$survival_time <- ifelse(age_cleaned_clinical$death_days_to == "[Not Applicable]", age_cleaned_clinical$last_contact_days_to, age_cleaned_clinical$death_days_to)
...
```{r}
age_cleaned_clinical$survival_time <- as.numeric(age_cleaned_clinical$survival_time)
```
...
```{r}
survival_object <- Surv(time = age_cleaned_clinical$survival_time,
                        event = age_cleaned_clinical$death_event)

fit_object <- survfit(survival_object ~ age_category, data = age_cleaned_clinical)
```
...
# 1. What does the y axis represent? The survival rate for each age group in age_cleaned_clinical, with 1 signifying that all patients survived.

# 2. What does the x axis represent? The number of days patients survive post-diagnosis

# 3. What age group appears to have the greatest rate of survival? The mid-aged group

# 4. What age group appears to have the worst rate of survival? The young group

# 5. What is the approx. 15-year survival rate for "Old" breast cancer patients? Approximately 0.31

# 6. What is the p-value? Is it statistically significant and what does that tell us about the relationship between age and breast cancer survival? The p-value is 0.27, which is significant because it is over 0.05. This indicates that the null hypothesis is true- there is no correlation between age and breast cancer survival.
